...
A small Node script (`out/stub.js`) launched as a hidden, real shell terminal via `shellPath`. Two purposes:
...
2. **IPC socket lifetime.** After writing its payload, the stub stays alive (`setInterval` heartbeat) for the lifetime of the dterm session, holding VS Code's per-terminal `VSCODE_IPC_HOOK_CLI` Unix socket bound. The daemon-side shell's env points at a workspace-scoped symlink that targets this socket, so `code` CLI invocations from the daemon-side shell follow the symlink to the live IPC socket. When the dterm session ends (or the extension deactivates), the extension calls `Terminal.dispose()` on the stub and the IPC socket goes with it.

The stub is launched through one of the `out/shims/{bash,zsh,fish,dterm}` symlinks; VS Code's shell-integration injection is keyed on the basename of `shellPath`, so launching via a symlink named `bash` makes VS Code inject `--init-file` for bash, etc.
