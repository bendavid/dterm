...
The stub is launched through one of the `out/shims/{bash,zsh,fish,dterm}` symlinks (each pointing directly at `stub.js`); VS Code's shell-integration injection is keyed on the basename of `shellPath`, so launching via a symlink named `bash` makes VS Code inject `--init-file` for bash, etc.
